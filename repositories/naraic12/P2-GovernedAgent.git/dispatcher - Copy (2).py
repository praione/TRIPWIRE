...
def init_agent(name: str):
    ...
    # Production agents communicate via Pub/Sub
    return {
        "agent_id": name,
        "type": "production",
        "role": agent_data.get("role_definition", {}).get("role_name"),
        "input_queue": agent_data.get("input_queue"),
        "output_queue": agent_data.get("output_queue"),
        "capabilities": agent_data.get("role_definition", {}).get("required_capabilities", [])
    }

def call_agent(agent: Dict, prompt: str) -> str:
    """Call production agent via Pub/Sub"""
    if isinstance(agent, dict) and agent.get("type") == "production":
        # Production agents - route via Pub/Sub
        publisher = pubsub_v1.PublisherClient()
        
        # Extract topic name from input_queue
        # Format: "pubsub://project/topics/agent_XXXXX_input"
        input_queue = agent.get("input_queue", "")
        if "topics/" in input_queue:
            topic_name = input_queue.split("topics/")[1]
        else:
            topic_name = input_queue
        ...
        # For now, return confirmation - in production, would wait for response
        return f"Task sent to {agent['agent_id']} via {topic_name}"
    
    # Fallback for any remaining test agents (remove this later)
    if hasattr(agent, "generate_rational_output"):
        return agent.generate_rational_output(prompt)
    
    return "Agent type not recognized"
