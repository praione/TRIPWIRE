...
def call_agent(agent: Dict, prompt: str, trace_id: str) -> str:
    ...
    # Use a filter to ensure we only get the message for our trace_id
    message_filter = f'attributes.trace_id = "{trace_id}"'
    ...
    try:
        ...
        with subscriber:
            ...
            if pull_response.received_messages:
                ...
                subscriber.acknowledge(subscription=temp_sub_path, ack_ids=[message.ack_id])
                ...
                raise TimeoutError(f"No response from agent {agent_id} within {timeout}s")
        ...
        try:
            subscriber.delete_subscription(subscription=temp_sub_path)
